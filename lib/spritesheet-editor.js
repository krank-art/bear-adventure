...
export default class SpritesheetEditor {
   ...
   */
  constructor({spritesheet, image, grid, tileSize = 8, scale = 1}) {
    ...
    this.setupSelection();
    if (this.spritesheet)
      this.update(this.spritesheet, this.tileSize);
  }

  update(spritesheet, tileSize = this.tileSize) {
    this.spritesheet = spritesheet;
    this.image.src = this.spritesheet.src;
  ...
  updateGrid() {
    this.grid.querySelectorAll('.col').forEach(el => el.remove());
    this.grid.querySelectorAll('.row').forEach(el => el.remove());
  ...
  updateSelection(x, y, w = 1, h = 1) {
    ...
    this.selection.w = w;
    this.selection.h = h;
    if (this.selection.x === null && this.selection.y === null) {
      this.selectionFrame.style.display = "none";
      return;
    }
