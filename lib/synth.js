function generateNotesWithFreq(detuneFactor = 0) {
  ...
  for (let octave = startOctave; octave <= endOctave; octave++) {
    for (let i = 0; i < notes.length; i++) {
      ...
      const detuneScale = detuneFactor > 0 ? 1 + (Math.random() * 2 - 1) * (detuneFactor / 1000) : 1;
      const wackFreq = freq * detuneScale;

      result[noteName] = +wackFreq.toFixed(2);
    }
  }
  return result;
}

const notes = generateNotesWithFreq(0);
